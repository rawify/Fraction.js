...
"s"in a&&(d*=BigInt(a.s));else if(0 in a)d=BigInt(a[0]),1 in a&&(b=BigInt(a[1]));else if("bigint"===typeof a)d=a;else throw v();f=d*b}else if("number"===typeof a){if(isNaN(a))throw v();0>a&&(f=-h,a=-a);if(0===a%1)f=d=BigInt(a);else if(0<a){c=1;var m=0,k=1,n=1;let t=1;1<=a&&(c=10**Math.floor(1+Math.log10(a)),a/=c);for(;1E7>=k&&1E7>=t;)if(b=(m+n)/(k+t),a===b){1E7>=k+t?(d=m+n,b=k+t):t>k?(d=n,b=t):(d=m,b=k);break}else a>b?(m+=n,k+=t):(n+=m,t+=k),1E7<k?(d=n,b=t):(d=m,b=k);d=BigInt(d)*BigInt(c);b=BigInt(b)}}else if("string"===
